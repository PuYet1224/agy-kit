...
// --- Main export ---
// skillName + note are passed separately so claude can use native slash command
export async function runAgent(prompt, workDir, agentName = 'gemini', timeout = 600000, skillName = null, note = null) {
  if (agentName === 'gemini') {
    return runGemini(prompt, timeout);
  }

  const bin = findBin(agentName);

  // Claude: use native slash command — claude loads .claude/skills/ automatically
  if (agentName === 'claude' && skillName) {
    const claudePrompt = `/${skillName}\n\n${note || ''}`;
    return runCLI(bin, claudePrompt, workDir, timeout);
  }

  // agy / codex / others: embed full skill content in prompt
